...
class AsyncLLMServerManager:
    ...
    def __init__(self, config: DictConfig, worker_group: RayWorkerGroup):
        ...
        # Track which servers are currently active
        self.active_server_indices = set(range(self.rollout_dp_size))
    ...
    def wake_up(self, server_indices: List[int] = None):
        """Wake up vllm instances.
        
        Args:
            server_indices: List of server indices to wake up. If None, wake up all servers.
        """
        if server_indices is None:
            server_indices = list(range(self.rollout_dp_size))
            
        indices_to_wake = [idx for idx in server_indices if idx not in self.active_server_indices]
        
        if indices_to_wake:
            if len(server_indices) == self.rollout_dp_size:
                # Full DP group wake-up - safe to use collective operations
                print(f"Waking up full DP group: {indices_to_wake}")
                ray.get([self.async_llm_servers[idx].wake_up.remote() for idx in indices_to_wake])
            else:
                # Partial wake-up - try individual wake-ups with timeout
                print(f"Attempting partial wake-up for servers: {indices_to_wake}")
                successful_wakeups = []
                for idx in indices_to_wake:
                    try:
                        # Use a timeout to avoid hanging indefinitely
                        # Pass tags to help coordinate with vLLM's DP architecture
                        ray.get(self.async_llm_servers[idx].wake_up.remote([str(idx)]), timeout=60)
                        successful_wakeups.append(idx)
                        print(f"Successfully woke up server {idx}")
                    except ray.exceptions.GetTimeoutError:
                        print(f"Error: Server {idx} wake_up timed out after 60 seconds")
                        print(f"This is likely due to collective operation coordination issues in vLLM DP")
                    except Exception as e:
                        print(f"Error waking up server {idx}: {e}")
        
        self.active_server_indices.update(server_indices)
        
        # Update scheduler with only active servers
        active_addresses = [self.server_addresses[idx] for idx in sorted(self.active_server_indices)]
        self.chat_scheduler.update_server_addresses(active_addresses)
        
        print(f"Active server indices after wake_up: {sorted(self.active_server_indices)}")

    def sleep(self, server_indices: List[int] = None):
        """Sleep vllm instances.
        
        Args:
            server_indices: List of server indices to sleep. If None, sleep all servers.
        """
        if server_indices is None:
            ray.get([server.sleep.remote() for server in self.async_llm_servers])
            self.active_server_indices.clear()
        else:
            ray.get([self.async_llm_servers[idx].sleep.remote() for idx in server_indices])
            self.active_server_indices.difference_update(server_indices)
            
        # Update scheduler with only active servers
        if self.active_server_indices:
            active_addresses = [self.server_addresses[idx] for idx in sorted(self.active_server_indices)]
            self.chat_scheduler.update_server_addresses(active_addresses)
        else:
            self.chat_scheduler.update_server_addresses([])
            
        print(f"Active server indices after sleep: {sorted(self.active_server_indices)}")
